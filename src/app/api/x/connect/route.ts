...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (!tokenData.refresh_token) {
      console.error("No refresh token received. Make sure 'offline.access' scope is requested.")
      return NextResponse.json(
        {
          error: "No refresh token received. Please ensure 'offline.access' scope is requested.",
        },
        { status: 400 },
      )
    }
    ...
    if (!userRes.ok) {
      ...
      return NextResponse.json({ error: userData.error || "Failed to get user profile" }, { status: 400 })
    }
