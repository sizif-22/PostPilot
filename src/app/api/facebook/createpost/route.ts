...
export async function POST(request: Request) {
  try {
    const { accessToken, pageId, message, scheduledDate, published, imageUrls, clientTimeZone }:{imageUrls:MediaItem[],accessToken:any, pageId:any, message:any, scheduledDate:any, published:any,clientTimeZone:any} = await request.json();
    ...
    try {
      ...
      const pageTokenResponse = await fetch(`https://graph.facebook.com/v19.0/${pageId}?fields=access_token,name&access_token=${accessToken}`);
      ...
      if (pageTokenData.access_token) {
        ...
        console.log('Using provided token (assuming it\'s already a page token)');
      }
    } catch (tokenError) {
      console.warn('Could not get page token, using provided token:', tokenError);
    }
    ...
    if (time.scheduled_publish_time) {
      ...
      if (time.scheduled_publish_time < minScheduledTime) {
        return NextResponse.json(
          { error: 'Scheduled time must be at least 13 minutes in the future (to account for processing time and network delays)' },
          { status: 400 }
        );
      }
    }
