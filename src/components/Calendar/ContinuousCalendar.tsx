...
export const ContinuousCalendar: React.FC<ContinuousCalendarProps> = ({ onEventSelect, highlightedDates = [] }) => {
        ...
        {week.map(({ month, day }, dayIndex) => {
          ...
          return (
            ...
            >
              ...
              {isNewMonth && (
                <span className="absolute bottom-1 left-1 w-[calc(100%-8px)] truncate text-xs font-semibold text-slate-300 sm:bottom-1.5 sm:left-1.5 sm:text-sm lg:bottom-2 lg:left-2 lg:text-base">
                  {monthNames[month]}
                </span>
              )}
              {hasEvents && (
  ...
  return (
    <div className="calendar-container px-4 text-slate-800 bg-white  h-[calc(100vh-2rem)] overflow-y-auto relative rounded-lg pb-4 shadow">
...
export const Select = ({ name, value, label, options = [], onChange, className }: SelectProps) => (
  <div className={`relative ${className}`}>
    ...
    <select
      ...
      value={value}
      onChange={onChange}
      className="cursor-pointer rounded-lg border border-gray-300 py-1 pl-2 pr-6 text-sm font-medium text-gray-900 bg-[#21212101] focus:outline-none sm:py-1.5 sm:pl-3 sm:pr-8 lg:py-2.5"
      required
    >
      {options.map((option) => (
