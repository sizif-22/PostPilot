...
const MemberDialog = ({ open, onOpenChange, member, onSubmit, title }: MemberDialogProps) => {
  ...
  return (
    ...
    >
      <div onClick={(e) => e.stopPropagation()} className="bg-white rounded-lg w-full max-w-md mx-4 shadow-xl overflow-hidden">
...
export const Team = () => {
  const [members, setMembers] = useState<TeamMember[]>([
    ...
    { id: '3', name: 'Mike Johnson', email: 'mike@example.com', role: 'inspector', status: 'active' },
    { id: '4', name: 'Sarah Wilson', email: 'sarah@example.com', role: 'contributor', status: 'pending', invitedAt: new Date() },
  ]);
      ...
      return (
        <div className="flex items-center gap-1 text-amber-600">
          ...
          <span className="text-xs">
            Pending ({invitedAt ? `Invited ${formatTimeAgo(invitedAt)}` : 'Not accepted'})
          </span>
        </div>
      );
    }
        ...
        <div className="rounded-lg border border-stone-200 overflow-hidden">
          <table className="w-full">
            <thead className="bg-stone-50 border-b border-stone-200">
              <tr>
                <th className="text-left py-3 px-4 text-sm font-medium text-stone-500">Name</th>
                  ...
                  <td className="py-3 px-4">
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-full bg-violet-100 flex items-center justify-center">
                        <span className="text-violet-700 font-medium">{member.name.charAt(0)}</span>
                      </div>
                      <span className={member.status === 'pending' ? 'text-stone-500' : ''}>{member.name}</span>
                    </div>
                  </td>
                  <td className="py-3 px-4">
                    <div className="flex items-center gap-2">
                      <FiMail className="w-4 h-4 text-stone-400" />
                      <span className={member.status === 'pending' ? 'text-stone-500' : ''}>{member.email}</span>
                    </div>
                  </td>
                  <td className="py-3 px-4">
