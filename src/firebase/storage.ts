...
const convertVideoToMp4 = async (file: File): Promise<File> => {
  try {
    ...
    console.log('Converting video:', file.name, 'Size:', Math.round(file.size / 1024 / 1024), 'MB');
    ...
    const mp4Url = result.secure_url.replace('/upload/', '/upload/f_mp4,vc_h264,ac_aac/');
...
const uploadImage = async ({
  ...
  try {
    ...
    const uploadTask = await FireStorage.uploadBytes(mainStorageRef, processedFile);
