...
export const getUser = (email: string, callback: (user: User | null) => void): fs.Unsubscribe => {
...
export const acceptJoiningToAChannel = async(notification:Notification, user:User) => {
  const channel = (await fs.getDoc(fs.doc(db,"Channels",notification.channelId))).data() as Channel;
  const tm = channel.TeamMembers.find((tm)=> tm.email == user.email && tm.status == "pending");
  if(tm){
    ...
    await fs.updateDoc(fs.doc(db,"Users",user.email),{channels:fs.arrayUnion({authority:tm.role , id:notification.channelId})});
    await fs.updateDoc(fs.doc(db,"Users",user.email),{notifications:fs.arrayRemove(notification)});
  }
}
export const rejectJoiningToAChannel = async(notification:Notification, user:User) => {
  const channel = (await fs.getDoc(fs.doc(db,"Channels",notification.channelId))).data() as Channel;
  const tm = channel.TeamMembers.find((tm)=> tm.email == user.email && tm.status == "pending");
  if(tm){
    await fs.updateDoc(fs.doc(db,"Channels",notification.channelId),{
