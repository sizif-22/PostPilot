...
const createPost = async (post: Post, channelId: string) => {
    ...
    const firestorePost: Partial<Post> = {
        id: post.id,
    ...
    if (post.scheduledDate) {
        ...
        firestorePost.scheduledDate = typeof post.scheduledDate === 'number' 
            ? post.scheduledDate 
...
const editPost = async (postId: string, channelId: string, updatedPost: Partial<Post>) => {
    // First, get the channel document to find the post
    const channelDoc = await fs.getDoc(fs.doc(db, "Channels", channelId));
    if (!channelDoc.exists()) {
        throw new Error("Channel not found");
    ...
    if (updatedPost.scheduledDate) {
        updatedPostData.scheduledDate = typeof updatedPost.scheduledDate === 'number' 
