...
import { FiLinkedin, FiCheck, FiAlertCircle, FiUser, FiUsers } from 'react-icons/fi';
import { linkedinChannedInterface } from '@/convex/connectCollection';
import { decrypt, encrypt, isValidEncryptedFormat } from '@/utils/encryption';
import { getOrganizations } from '../server-actions/linkedIn/get-organizations';
...
export default function LinkedInCallbackPage() {
  ...
  const connectLinkedIn = useMutation(api.connectCollection.connectLinkedIn);
  useEffect(() => {
    const handleLinkedInAuth = async () => {
      ...
      try {
        const data = await getOrganizations(code);
  ...
  const handleAccountSelect = async () => {
    ...
    try {
      ...
      let linkedinData: linkedinChannedInterface;
      ...
      if (selectedAccount.type === 'personal') {
        ...
        } as linkedinChannedInterface;
      }

      await connectLinkedIn({ collectionId, linkedInChannel: linkedinData });
      // Clean up cookies
      ...
      if (error instanceof Error) {
        ...
        setError('An unknown error occurred while saving the account selection.');
      }
  ...
  const allAccounts: LinkedInAccount[] = [...(personalAccount ? [personalAccount] : []), ...organizations];
      ...
      <div className="min-h-screen bg-gray-50 dark:bg-darkBackground flex items-center justify-center p-4">
        <div className="bg-white dark:bg-secondDarkBackground rounded-lg p-8 max-w-md w-full shadow-lg dark:shadow-[0_4px_32px_0_rgba(0,0,0,0.45)]">
          ...
          <div className="text-gray-600 dark:text-gray-400 mb-6">
            ...
            <p className="text-sm">Please make sure you have granted the necessary permissions and try again.</p>
          </div>
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-darkBackground flex items-center justify-center p-4">
      <div className="bg-white dark:bg-secondDarkBackground rounded-lg p-8 max-w-2xl w-full shadow-lg dark:shadow-[0_4px_32px_0_rgba(0,0,0,0.45)]">
        <div className="flex items-center gap-3 mb-6">
          ...
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Select LinkedIn Account</h1>
        </div>

        <p className="text-gray-600 dark:text-gray-400 mb-6">
          Choose which LinkedIn account you want to connect to your channel. You can connect to your personal account or
          any organizations where you are an administrator.
        </p>
        ...
        {allAccounts.length === 0 ? (
          <div className="text-center py-8">
            ...
            <div className="text-gray-600 dark:text-gray-400">
              ...
              <p className="text-sm">
                Please make sure you have access to your LinkedIn account or are an administrator of at least one
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <h3 className="font-medium text-gray-900 dark:text-white">{personalAccount.name}</h3>
                          <p className="text-sm text-gray-500 dark:text-gray-400">
                            {personalAccount.headline || 'Personal LinkedIn Account'}
                          </p>
                          {personalAccount.profileUrl && (
                            <p className="text-xs text-blue-600 dark:text-blue-400">{personalAccount.profileUrl}</p>
                          )}
                        </div>
                      </div>
                      {selectedAccount?.id === personalAccount.id && (
                        <FiCheck className="text-blue-500 text-xl shrink-0" />
                      )}
                    </div>
                  </div>
                </>
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            {org.profileUrl && (
                              <p className="text-xs text-blue-600 dark:text-blue-400">{org.profileUrl}</p>
                            )}
                          </div>
                        </div>
                        {selectedAccount?.id === org.id && <FiCheck className="text-blue-500 text-xl shrink-0" />}
                      </div>
                    </div>
                  ))}
                </>
