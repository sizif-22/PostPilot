...
function XConnectV1PageComponent() {
  const router = useRouter();
  const searchParams = useSearchParams();
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const connectX = useMutation(api.connectCollection.connectXV1);
  useEffect(() => {
    const handleV1Auth = async () => {
      ...
      try {
        ...
        await connectX({
          collectionId,
          xV1: { v1aAccessToken: encryptedV1aAccessToken, v1aAccessSecret: encryptedV1aAccessSecret },
        });
        ...
        if (error instanceof Error) {
          ...
          setError('Failed to connect to X v1');
        }
        setLoading(false);
      }
    };
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-darkBackground flex items-center justify-center p-4">
      <div className="bg-white dark:bg-secondDarkBackground rounded-lg p-8 max-w-2xl w-full shadow-lg dark:shadow-[0_4px_32px_0_rgba(0,0,0,0.45)]">
        <div className="flex items-center gap-3 mb-6">
          ...
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Connect X for Video (Step 2 of 2)</h1>
        </div>

        <p className="text-gray-600 dark:text-gray-400 mb-6">
          To enable video uploads, an additional connection is required. This is due to changes in the X API. Please
