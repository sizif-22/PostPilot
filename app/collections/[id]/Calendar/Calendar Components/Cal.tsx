        ...
        {week.map(({ month, day }, dayIndex) => {
          ...
          return (
            <div
              ...
              data-month={month}
              data-day={day}
              className={`relative group border font-medium transition-all hover:z-20  hover:border-violet-400 dark:hover:border-violet-600 border-[#00000005] dark:border-stone-700 w-full min-h-[50px] sm:min-h-[80px] lg:min-h-[200px] px-2 pt-12 pb-2 ${hasEvents
              ...
              <span
                className={`absolute left-1 top-1 flex size-6 items-center rounded-full justify-center text-xs sm:size-7 sm:text-sm lg:left-2 lg:top-2 lg:size-8 lg:text-base ${isToday
                ...
                <div className="absolute top-8 sm:top-12 lg:top-14 left-1 right-1 lg:left-2 lg:right-2 flex flex-col gap-1 overflow-y-auto max-h-[calc(100%-40px)] sm:max-h-[calc(100%-50px)] lg:max-h-[calc(100%-60px)]">
                  <ReactSortable
                    ...
                    {postsForDay.slice(0, 2).map((post) => (
                      <div
                        key={post.id}
                        className={` ${membership?.role == "Reviewer" && "disable-sortable"
                          }  `}>
                        <PostCard
